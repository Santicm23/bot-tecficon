    ...
    def read_sinester_selenium(self, id_siniestro: int) -> Siniestro:
        ...
        try:
            self.driver.find_element(
                By.XPATH, f"//*[contains(text(), '{id_siniestro}')]"
            ).click()
        except:
        ...
        ciudad = self.driver.find_element(By.ID, "incidentCity").get_attribute("value")

        dpto = Select(self.driver.find_element(By.ID, "incidentLocation_node1"))
        dpto = dpto.first_selected_option.text

        producto = self.driver.find_element(By.ID, "productName").get_attribute("value")

        fecha_accidente = self.str_optional_to_str(
            self.driver.find_element(By.ID, "incidentDate").get_attribute("value")
        )

        fecha_accidente = self.format_date(fecha_accidente)

        direccion_ocurrencia = self.driver.find_element(
            By.ID, "incidentAddress"
        ).get_attribute("value")

        tipo_encargo = self.driver.find_element(By.ID, "procTypeDesc").get_attribute(
            "value"
        )

        tipo_encargo = tipo_encargo.lower() if tipo_encargo else ""

        desc_instancia_procedimiento = self.str_optional_to_str(
            self.driver.find_element(By.ID, "procInstaDesc").get_attribute("value")
        ).split(" - ")
        ...
        tramitador = self.driver.find_element(By.ID, "tramitName").get_attribute(
            "value"
        )

        mediador = self.driver.find_element(By.ID, "agentName").get_attribute("value")

        tomador = self.driver.find_element(By.ID, "holderName").get_attribute("value")

        propietario = self.driver.find_element(By.ID, "ownerName").get_attribute(
            "value"
        )
        ...
        siniestro = Siniestro(
            ...
            direccion_ocurrencia=self.str_optional_to_str(direccion_ocurrencia),
